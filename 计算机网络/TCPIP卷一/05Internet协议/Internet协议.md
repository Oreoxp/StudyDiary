...
#### **1. 背景与挑战**
- **多宿主与双栈环境**：主机可能配置多个IP地址（IPv4/IPv6），需选择最优源地址和目的地址。
- **核心问题**：
  - **源地址选择**：发送数据报时，从多个可用地址中选择最合适的源地址。
  - **目的地址选择**：当目的主机有多个地址时（如IPv4和IPv6），选择最优目的地址。
- **目标**：避免非对称路由、分组丢弃或过滤，优化通信效率。

---

#### **2. 地址选择的核心原则（RFC 3484）**
| **原则**         | **描述**                                                     |
| ---------------- | ------------------------------------------------------------ |
| **地址范围匹配** | 优先选择源地址与目的地址范围相同的地址对（如链路本地地址对链路本地地址）。 |
| **避免临时地址** | 优先使用永久地址（如全球地址）而非临时地址（如隐私扩展地址）。 |
| **最长前缀匹配** | 源地址与目的地址的通用前缀越长，优先级越高。                 |
| **标签匹配**     | 源地址和目的地址的标签（L(A)）一致时优先选择。               |
| **接口匹配**     | 优先选择与发送接口绑定的地址。                               |

---

#### **3. 源地址选择算法**
源地址候选集合 `CS(D)` 根据目的地址 `D` 生成，按以下规则排序：

1. **相同地址优先**：若源地址等于目的地址（`A = D`），优先级最高。
2. **范围匹配**：
   - 源地址范围应不小于目的地址范围（如全球地址优先于链路本地地址）。
   - 示例：目的地址为全球地址时，优先选择全球源地址。
3. **避免过期地址**：优先选择“首选”（非过期）地址。
4. **移动IP支持**：优先选择家乡地址（HoA）而非转交地址（CoA）。
5. **接口绑定**：优先选择与发送接口关联的地址。
6. **标签匹配**：源地址标签与目的地址标签一致时优先级高。
7. **非临时地址优先**：优先使用非临时地址。
8. **最长前缀匹配**：源地址与目的地址的通用前缀越长，优先级越高。

---

#### **4. 目的地址选择算法**
目的地址候选集合 `SD(S)` 根据源地址 `S` 生成，按以下规则排序：

1. **可达性优先**：排除不可达或无法选择源地址的目的地址。
2. **范围匹配**：目的地址范围应与源地址范围一致。
3. **避免过期地址**：优先选择“首选”地址。
4. **移动IP支持**：优先选择家乡地址（HoA）。
5. **标签匹配**：目的地址标签与源地址标签一致时优先级高。
6. **策略优先级**：根据策略表中的优先级（`P(A)`）排序。
7. **本地传输优先**：优先选择无需隧道封装的目的地址。
8. **最小范围优先**：在相同策略下，选择更小范围的地址（如链路本地优先于全球）。
9. **最长前缀匹配**：目的地址与源地址的通用前缀越长，优先级越高。

---

#### **5. RFC 3484 默认策略表（IPv6）**
| **地址前缀**                | **优先级 `P()`** | **标签 `L()`** | **说明**                   |
| --------------------------- | ---------------- | -------------- | -------------------------- |
| `::1/128`（环回地址）       | 50               | 0              | 最高优先级，用于本地通信。 |
| `::/0`（默认路由）          | 40               | 1              | 全球单播地址。             |
| `2002::/16`（6to4）         | 30               | 2              | 隧道地址，优先级较低。     |
| `::/96`（IPv4兼容）         | 20               | 3              | 历史遗留，不推荐使用。     |
| `::ffff:0:0/96`（IPv4映射） | 10               | 4              | IPv4映射地址，最低优先级。 |

---

#### **6. 实际应用与挑战**
- **双栈主机（IPv4/IPv6）**：
  - 优先选择IPv6地址（若可用），避免NAT兼容性问题。
  - 示例：访问 `www.example.com` 时优先使用IPv6地址。
- **唯一本地地址（ULA）**：
  - RFC 4193 定义的ULA（如 `fd00::/8`）在专用网络中使用，默认优先级低于全球地址。
  - **RFC 3484 修订**：优化ULA的优先级策略，确保其在专用网络中优先于IPv4地址。
- **移动IP场景**：
  - 家乡地址（HoA）优先级高于转交地址（CoA），确保通信连续性。

---

#### **7. 示例场景**
- **场景1**：双栈主机访问双栈服务器。
  - **源地址选择**：优先选择IPv6全球地址（如 `2001:db8::1`）。
  - **目的地址选择**：优先选择服务器的IPv6地址。
- **场景2**：移动节点（MN）在外部网络通信。
  - **源地址选择**：使用家乡地址（HoA）以维持会话。
  - **目的地址选择**：直接通信时优先选择CN的全球地址。

---

#### **8. 修订与改进（RFC 3484bis）**
- **唯一本地地址（ULA）处理**：
  - 明确ULA在专用网络中的优先级，避免与全球地址冲突。
  - 示例：在内部网络中，ULA `fd12:3456::1` 优先于IPv4地址。
- **DNS轮询问题**：
  - 优化目的地址选择规则，减少因DNS轮询导致的非对称路由。

---

#### **9. 总结**
- **地址选择**是主机在多宿主和双栈环境中高效通信的关键。
- **RFC 3484** 提供标准化规则，结合最长前缀匹配、范围优先级等策略，确保地址选择最优。
- **实际部署**需结合网络策略（如防火墙规则、移动IP配置）进行调整，平衡安全性与灵活性。

通过合理配置地址选择策略，主机能够在复杂网络环境中实现高效、可靠的通信。 🌐🔗
