...
首先是线程的启动过程：
   ...
   3. base::RunLoop 类在创建消息循环的过程中，会通过成员函数 Init 创建用来一个用来消息循环的MessagePump 对象。

   4. 消息循环创建完成之后，调用 **`RunLoop`**类的成员函数 Run 进入消息循环。

   5. MessageLoop 类的成员函数 Run 创建一个RunLoop对象，并且调用它的成员函数Run进入消息循环。注意，该RunLoop对象在创建的过程，会关联上当前线程使用的消息循环，也就是创建它的MessageLoop对象。

   6. RunLoop类的成员函数Run负责建立好消息循环的嵌套关系，也就是设置好它的成员变量 previous_loop_ 和run_depth_ 等，然后就会调用其关联的 MessageLoop 对象的成员函数 RunHandler 进入消息循环。

   7. MessageLoop 类的成员函数 RunHandler 调用成员变量 pump_ 描述的一个 MessagePump 对象的成员函数Run进入消息循环。
